# astra-child_0204（Static Local 配信）
...
2. この子テーマ `astra-child_0204` をアップロードして有効化
...
※ `contact` は WordPress 側（`admin-post.php`）で受けて `wp_mail()` で送信します（送信先: `hiroto.shimokawa@blooo.co.jp`）。
...
- `assets/site/` 配下に、既存の `stitch_yume_site1New/` を **構造そのまま** 同梱しています
  ...
  - `shared/`, `image2/`, `image3/`, `image4/`, `Image/` など

## ZIP化（任意）

`astra-child_0204/scripts/zip-theme.sh` を実行すると、親ディレクトリに `astra-child_0204.zip` を作成します。
